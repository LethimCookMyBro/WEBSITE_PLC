 ...
 * Main JavaScript Application v2.0
 *
 * Features:
 * - Bilingual support (TH/EN) with i18n dictionary
...
const i18n = {
  en: {
    ...
    plan_starter_name: "Maintenance Essentials",
  ...
  th: {
    ...
    contact_location_text: "กรุงเทพมหานคร (ศูนย์ APAC)",
...
const I18n = {
  updateAll() {
    const translations = i18n[AppState.currentLang];
...
const UI = {
  ...
  updateMobileMenu() {
    ...
    mobileMenuBtn.classList.toggle(
      "l-navbar__menu-btn--active",
      AppState.isMobileMenuOpen,
    );
    mobileMenu.classList.toggle(
      "l-navbar__mobile-menu--active",
      AppState.isMobileMenuOpen,
    );
    mobileMenuBtn.setAttribute("aria-expanded", AppState.isMobileMenuOpen);
    mobileMenu.setAttribute("aria-hidden", !AppState.isMobileMenuOpen);
  ...
  updateLangToggle() {
    ...
    langButtons.forEach((btn) => {
      const isActive = btn.getAttribute("data-lang") === AppState.currentLang;
...
const Animator = {
  ...
  makeVisibleImmediately() {
    document
      .querySelectorAll(
        ".c-glass-card, .l-timeline__item, .l-metric__card, .l-pricing__card, .l-faq__item",
      )
      .forEach((el) => {
        el.style.opacity = "1";
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = { i18n, AppState, I18n, UI };
}
